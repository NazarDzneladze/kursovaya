...
public class ClaimsDao {
	...
	public int updateClaimRequest(Claims claimReq) {
		String sqlQuery = "UPDATE claims set `claim_status` = ? where `id` = ?;";
		try {
			PreparedStatement pstmt = (PreparedStatement) connection.prepareStatement(sqlQuery);
			pstmt.setInt(1, claimReq.isClaimStatus() ? 1 : 0);
			pstmt.setInt(2, claimReq.getId());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int deleteClaimRequest(Claims claimReq) {
		String sqlQuery = "DELETE FROM claims where `id` = ?;";
		try {
			PreparedStatement pstmt = (PreparedStatement) connection.prepareStatement(sqlQuery);
			pstmt.setInt(1, claimReq.getId());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (SQLException e) {
			e.printStackTrace();
