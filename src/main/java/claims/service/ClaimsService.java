...
public class ClaimsService {
	...
	public AppResponse<Integer> updateClaimRequest(int claimId, String status) {
		List<Claims> claims = claimsDao.getAllClaims("");
		int statusId = 0;
		for (Claims claim :
				claims) {
			if (claim.getId() == claimId) {
				claim.setClaimStatus(status.equals("true"));
				statusId = claimsDao.updateClaimRequest(claim);
			}
		}

		return new AppResponse<Integer>(statusId, false);
	}

	public AppResponse<Integer> deleteClaimRequest(int claimId) {
		List<Claims> claims = claimsDao.getAllClaims("");
		int statusId = 0;
		for (Claims claim :
				claims) {
			if (claim.getId() == claimId) {
				statusId = claimsDao.deleteClaimRequest(claim);
			}
		}
